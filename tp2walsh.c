...
void walsh(){
	...
	division = ligne / 2;
	i = 0;
	j = n + 2;		// on commence le calcul à la colonne n + 1 (après celle de f)
	while (j < colonne)		// calcul de la ANF
	{
		if (compteur < division)
		{
			tab[i][j] = (tab[i][j - 1]+tab[i+division][j-1]) ;		// on recopie les valeurs (première étape de l'Algorithme papillon)
			compteur++;
		}
		else if (compteur < division + division)		// addition des termes entre eux
		{
			compteur++;
			tab[i][j] = (tab[i-division][j-1]-tab[i][j-1]);
		}
